  ...
  if ((s_lab == e_lab) && (s_code == e_code)) {
    ...
    if (parser_state_tos->col_1 && !settings.format_col1_comments) {
      ...
      if ((parser_state_tos->ind_level <= 0) &&
        ...
        start_column =
            compute_code_target(*paren_targ) - settings.unindent_displace;
        if (start_column < 0) {
          start_column = 1;
        }
      }
    ...
    if (s_code != e_code) {
      start_column =
          count_columns(compute_code_target(*paren_targ), s_code, NULL_CHAR);
    } else {
    ...
    while (!had_eof) {
      ...
      case TAB:
        ...
        if (format) {
          ...
          if ((e_com == s_com) || (e_com[-1] != ' ') || (e_com - 1 == s_com) ||
              (e_com[-2] == '.')) {
            *e_com++ = ' ';
            column++;
          }
          ...
          *e_com++ = ' ';
          ...
          int tab_width =
              (settings.tabsize -
               ((column + found_column - start_column - 1) % settings.tabsize));
          column += tab_width;
